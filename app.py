...
with st.sidebar:
    ...
    if enable_critique:
        ...
        # 互斥逻辑：根据 last_action 决定对象
        if last_action == "eval" and st.session_state.get('e_report'):
            options = ["评审报告"]
        ...
        with st.spinner(f"正在调动【{mentor_type}】进行深度构思与 800 字试写..."):
            try:
                ...
                if res.text:
                    ...
                    if len(parts) > 1:
                        st.session_state['last_snippet'] = parts[1].strip()
                    else:
                        # 兜底：如果没切开，把后半段强制识别为片段，或提示未生成
                        st.session_state['last_snippet'] = "（引擎未产出独立片段，请在大纲中查看立意）"
                
                    save_to_nal_archive("creative", c_filename, full_content)
                        ...
                        * **原创性评估**：8/10分。设定新颖，视角独特。
                        ...
                        try:
                            ...
                            if res_obj.candidates and res_obj.candidates[0].content.parts:
                                ...
                                st.error("⚖️ 评审报告生成失败。文本可能触发了模型的安全过滤机制，请检查内容。")
                        except Exception as e:
                            st.error(f"评审发生异常: {e}")
                            
                        if 'res_obj' in locals() and res_obj.text:
                            # 记录最后一次操作是“评审”
